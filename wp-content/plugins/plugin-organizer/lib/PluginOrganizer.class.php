...
class PluginOrganizer {
	...
	function get_custom_meta_links($meta, $file) {
		if ($this->pluginBase == $file) {
			$meta[] = '<a href="http://www.jsterup.com/dev/wordpress/plugins/plugin-organizer/documentation/" target="_blank">Documentation</a>';
			$meta[] = '<a href="http://www.jsterup.com/dev/wordpress/plugins/plugin-organizer/faq/" target="_blank">FAQ</a>';
		}
		return $meta;
	}

	function display_admin_debug() {
		if (get_option('PO_display_debug_msg') == 1) {
			...
			if (array_diff($debugRoles, $roles) !== $debugRoles && get_class($PluginOrganizerMU) == 'PluginOrganizerMU' && sizeof($PluginOrganizerMU->debugMsg) > 0) {
				?>
				<div class="notice notice-warning" id="po_debug_msg_container" style="padding: 20px;">
					...
					if (current_user_can('activate_plugins')) {
						?>
						<a href="#" id="PO_disable_debug_msg">Disable Debug Messages</a>
						<script type="text/javascript" language="javascript">
							jQuery('#PO_disable_debug_msg').click(function() {
								jQuery.post(encodeURI(ajaxurl + '?action=PO_disable_debug_msg'), {PO_nonce: '<?php print $this->nonce; ?>'}, function (result) {
									jQuery('#po_debug_msg_container').remove();
								});
								return false;
							});
						</script>
	...
	function display_footer_debug() {
		if (get_option('PO_display_debug_msg') == 1) {
			...
			if (array_diff($debugRoles, $roles) !== $debugRoles && get_class($PluginOrganizerMU) == 'PluginOrganizerMU' && sizeof($PluginOrganizerMU->debugMsg) > 0) {
				?>
				<div id="po_debug_msg_container" style="position: relative;z-index: 99999;background: #fff;width: 100%;border: 4px solid #000;padding: 10px;">
					<div style="font-weight: bold;margin-bottom:10px;">Plugin Organizer Debug Messages</div>
					<hr>
					<?php
					foreach($PluginOrganizerMU->debugMsg as $debugMsg) {
	...
	function check_version() {
		...
		if (get_option("PO_version_num") != "9.5" && !in_array($pagenow, array("plugins.php", "update-core.php", "update.php"))) {
			$this->activate();
		}
	}
	...
	function activate() {
		...
		if (get_option("PO_version_num") != "9.5") {
			update_option("PO_version_num", "9.5");
		}
	...
	function compatibility_notices() {
		$activePlugins = get_option("active_plugins");
		if (in_array('wp-spamshield/wp-spamshield.php', $activePlugins)) {
			?>
			<div class="notice notice-error" id="PO_compat_notices" style="padding: 20px;">
				<strong>WARNING:</strong> You are currently running WP Spamshield. This plugin was removed from the wordpress plugin repository for containing malicious code that targeted other developers. You should immediately remove this plugin. It has attempted to modify the settings for Plugin Organizer as well as other plugins. Because of this malicious activity you can't run Plugin Organizer and WP Spamshield together. <a href="#" id="PO_disable_compat_notices">Disable this warning</a>
			</div>
			<script type="text/javascript" language="javascript">
				jQuery('#PO_disable_compat_notices').click(function() {
					jQuery.post(encodeURI(ajaxurl + '?action=PO_disable_compat_notices'), {PO_nonce: '<?php print $this->nonce; ?>'}, function (result) {
						jQuery('#PO_compat_notices').remove();
					});
					return false;
				});
			</script>
	...
	function save_post_meta_box($postID) {
		...
		if ((defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) || wp_is_post_revision($postID) || !current_user_can('edit_post', $postID) || ($ptSettingsFound !=1 && !current_user_can('activate_plugins')) || !in_array(get_post_type($postID), $supportedPostTypes) || (!isset($this->postedData['poSubmitPostMetaBox']) && $ptSettingsFound !=1)) {
			return $postID;
		}
