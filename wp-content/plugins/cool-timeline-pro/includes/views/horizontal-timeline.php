...
if ($ctl_loop->have_posts()) {
    ...
    while ($ctl_loop->have_posts()) : $ctl_loop->the_post();
        ...
        $ctl_story_color = get_post_meta($post->ID, 'ctl_story_color', true);
        $container_cls=isset($img_cont_size)?$img_cont_size:"full";
        $ctl_format_html='';
        $story_id="story-id-".$post->ID;
         $i++;
          $s_styles.=CooltimelineStyles::ctl_h_story_styles($post->ID);
         
        $posted_date=ctl_get_story_date($post->ID,$date_formats);
     ...
     if($active_design=='design-3'||$active_design=='design-4') {
            $dates_li .= ' <li class="ht-dates-'.esc_attr($design).'" id="' . esc_attr($story_id ). '" data-date="' . esc_attr($story_id ). '">'.$clt_icon.'<span class="ctl-story-time ' . esc_attr($selected ). '"  data-date="' .esc_attr($story_id). '" ><div class="ctl-tooltips"><span>'. apply_filters('ctl_story_dates',$posted_date).'</span></div></li>';
            }else{
             $dates_li .= ' <li  id="' . esc_attr($story_id ). '" class="ht-dates-'.esc_attr($design).'" data-date="' . esc_attr($story_id ). '">'.$clt_icon.'<span class="ctl-story-time ' . esc_attr($selected ). '"  data-date="' .esc_attr($story_id). '" >'. apply_filters('ctl_story_dates',$posted_date).'</li>';
            }
         }
...
$clt_hori_view .= '<div class="'.implode(" ",$main_wrp_cls).'"  id="'.esc_attr($timeline_wrp_id).'" data-rtl="'.$rtl.'" date-slider="ctl-h-slider-'.esc_attr($timeline_id).'" data-nav="nav-slider-'.esc_attr($timeline_id).'" data-items="'.esc_attr($items).'">
